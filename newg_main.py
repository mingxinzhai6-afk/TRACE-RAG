...
def build_graph_config(raw_yaml: dict, graph_method: str, dataset_name: str) -> Config:
    ...
    top_level = {k: v for k, v in raw_yaml.items()
                 ...
                 and k not in ("newg", "llm", "embedding", "chunk")}
    ...
    if abl_tag:
        final["exp_name"] = f"NewG_{graph_method}_{text_tag}_{model_short}_{abl_tag}".rstrip("_")
    else:
        final["exp_name"] = f"NewG_{graph_method}_{text_tag}_{model_short}".rstrip("_")
...
async def run_query_loop(query_dataset, engine, result_dir: str, eval_limit: int):
    ...
    if _HAS_TQDM:
        with _tqdm(total=n, desc="NewG", unit="q") as pbar:
...
async def main(args):
    ...
    # Extract ablation tag from YAML filename (e.g. NewG_abl_simple -> abl_simple)
    abl_tag = opt_path.stem.replace("NewG", "").strip("_")
    if abl_tag:
    ...
    newg_cfg = raw_yaml.get("newg", {})
    logger.info("=" * 60)
    logger.info("NewG Agentic Graph+Text RAG")
    logger.info("=" * 60)
    logger.info(f"  graph_method  = {args.graph_method}")
    logger.info(f"  text_method   = {args.text_method}")
    logger.info(f"  dataset       = {args.dataset_name}")
    logger.info(f"  max_rounds    = {newg_cfg.get('max_rounds', 3)}")
    logger.info(f"  modules       = critic={newg_cfg.get('use_critic')}, "
                f"commendor={newg_cfg.get('use_commendor')}, "
                f"norm={newg_cfg.get('use_normalizer')}, "
                f"disambig={newg_cfg.get('use_disambiguation')}")
    logger.info("=" * 60)
    ...
    # Construct NewG engine
    from Core.Query.NewGQuery import NewGEngine
    engine = NewGEngine(
    ...
    # Print stats
    try:
        from Core.Query.Commendor import get_commendor_stats
        from Core.Query.NewGQuery import get_route_stats
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NewG Agentic Graph+Text RAG")
    parser.add_argument("-opt", required=True, help="Path to NewG YAML")
