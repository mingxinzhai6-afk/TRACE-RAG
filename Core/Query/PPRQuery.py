...
class PPRQuery(BaseQuery):
    ...
    async def generation_qa(self, query, context):
        ...
        # Short-form factoid QA (PopQA, HotpotQA, etc.)
        if getattr(self.config, 'response_type', '') == 'short-form':
            retrieved_passages = context[:self.config.num_doc]
            report_data = '\n\n'.join(retrieved_passages) if retrieved_passages else ''
            msg = QueryPrompt.SHORT_FORM_QA_RESPONSE.format(report_data=report_data)
            try:
                return await self.llm.aask(msg=query, system_msgs=[msg])
            except Exception as e:
