...
class BasicQuery(BaseQuery):
    ...
    async def generation_qa(self, query, context):
        if context is None:
            return QueryPrompt.FAIL_RESPONSE
        # Short-form factoid QA — check first, covers tree_search too
        if self.config.response_type.lower() in ("short-form", "short_form", "short", "factual"):
            if self.config.tree_search:
                instruction = (
                    f"Context:\n{context}\n\n"
                    f"Question: {query}\n\n"
                    "Answer with the single most standard label only (1-3 words max). "
                    "No adjectives, no 'and' combinations, no explanation. "
                    "Example: output 'politician' not 'Canadian politician'."
                )
                return await self.llm.aask(msg=instruction)
            response = await self.llm.aask(
                query,
                system_msgs=[QueryPrompt.SHORT_FORM_QA_RESPONSE.format(report_data=context)],
            )
            return response
        if self.config.tree_search:
