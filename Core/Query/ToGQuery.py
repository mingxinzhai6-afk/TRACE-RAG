...
class ToGQuery(BaseQuery):
    ...
    async def _retrieve_relevant_contexts(self, query: str, mode: str = "half_stop") -> str:
        if mode == "retrieve":
            ...
            context = answer_prompt.replace('{}', query + '\n')
            chain_prompt = '\n'.join(
                [', '.join([str(x) for x in chain]) for sublist in self.reasoning_paths_list for chain in sublist])
            context += "\nKnowledge Triplets: " + chain_prompt + 'A: '
    ...
    async def _generate_final_answer(self, query):
        """Generate the final answer using answer_prompt with all collected reasoning paths."""
        from Core.Prompt.TogPrompt import answer_prompt
        context = answer_prompt.replace('{}', query + '\n')
        chain_prompt = '\n'.join(
            [', '.join([str(x) for x in chain]) for sublist in self.reasoning_paths_list for chain in sublist])
    ...
    async def query(self, query):
        ...
        for depth in range(1, self.config.depth + 1):
            ...
            if context:
                ...
                if is_stop:
                    # Evaluate said "Yes" — enough knowledge collected.
                    # But response is the evaluate output, NOT the final answer.
                    # Generate the actual answer using answer_prompt.
                    logger.info("ToG stopped at depth %d." % depth)
                    response = await self._generate_final_answer(query)
                    break
                else:
            ...
            # for-else: all depths exhausted without break → generate final answer
            logger.info("ToG exhausted all depths, generating final answer.")
            response = await self._generate_final_answer(query)
        ...
        if getattr(self.config, 'use_critic', False) and response:
            ctx_str = str(self.reasoning_paths_list)
            response, _ = await self._critic_loop(query, ctx_str, response)
    ...
    async def generation_qa(self, mode: str, context: str) -> Union[tuple[bool, str], str]:
        response = await self.llm.aask(
            msg=context,
            system_msgs=["You are an AI assistant that helps people find information."],
        )
