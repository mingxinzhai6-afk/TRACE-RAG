"""
NewGQuery - unified Graph+Text RAG engine aligned with the paper pipeline.
...
from Core.Query.QueryUnderstanding import QueryUnderstanding, QueryAction
from Core.Query.HybridFusion import (
    rrf_fuse,
    normalize_graph_output,
    normalize_text_output,
)
from Core.Query.EntityDisambiguation import EntityDisambiguation
from Core.Query.EvidenceFusion import EvidenceFusionModule
from Core.Query.ReGenerationAgent import ReGenerationAgent
from Core.Query.Commendor import Commendor
...
class NewGEngine:
    ...
    async def query(self, question: str) -> Dict[str, Any]:
        """Run the full NewG pipeline for a single question."""
        result = {
            "output": "",
            "selection_history": [],
            "rounds": 0,
    ...
    def _candidate_entities(text: str) -> List[str]:
        ...
        for match in re.finditer(r"'([^']{2,100})'", text or ""):
            ...
            if ent and NewGEngine._looks_like_quoted_entity(ent):
                entities.append(ent)
        return entities
    ...
    def _restore_entity_case(key: str, evidence: str) -> str:
        for ent in NewGEngine._candidate_entities(evidence[:3000]):
            if NewGEngine._entity_key(ent) == key:
                return ent
        return key
    ...
    def _is_bad_bridge_entity(entity: str) -> bool:
        ...
        if lower.startswith(("#", "round", "candidate", "answer")):
            return True
        if NewGEngine._looks_like_relation_phrase(lower):
            return True
        return False
    ...
    def _looks_like_quoted_entity(text: str) -> bool:
        ...
        if len(lower.split()) > 7:
            return False
        if NewGEngine._answer_category(lower) in {"temporal", "number", "ranking"}:
            return False
