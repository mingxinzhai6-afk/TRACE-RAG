...
class GraphRAG(ContextMixin, BaseModel):
    ...
    async def _build_retriever_context(self):
        ...
        try:
            items = [(k, v) for k, v in self._retriever_context.items() if v]
            for context_name, use_context in tqdm(items, desc="Building retriever context", unit="component", leave=True):
