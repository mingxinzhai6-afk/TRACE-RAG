...
class ERGraph(BaseGraph):
    ...
    async def _named_entity_recognition(self, passage: str):
        ...
        # 若 LLM 返回的是 JSON 字符串而非列表，解析为列表，避免被按字符遍历
        if isinstance(entities, str):
            try:
                entities = json.loads(entities) if entities.strip() else []
            except (json.JSONDecodeError, TypeError):
                entities = [e.strip() for e in entities.split(",") if e.strip()] if entities else []
    ...
    async def _build_graph_from_tuples(entities, triples, chunk_key):
        ...
        for _entity in entities:
            ...
            if isinstance(_entity, dict):
                _entity = _entity.get("name", _entity.get("entity", "")) or ""
            if not isinstance(_entity, str):
                _entity = str(_entity).strip()
            entity_name = clean_str(_entity)
            # 跳过空或单字符标点等无效实体，减少无意义告警
            if not entity_name or (len(entity_name) == 1 and entity_name in '[],"\' \t'):
                logger.debug(f"entity name is not valid, entity is: {repr(_entity)}, so skip it")
                continue
            entity = Entity(entity_name=entity_name, source_id=chunk_key)
            maybe_nodes[entity_name].append(entity)

        for triple in triples:
            # 支持 dict 格式: {'subject': ..., 'predicate': ..., 'object': ...}
            if isinstance(triple, dict):
                triple = [
                    triple.get("subject", triple.get("Subject", "")),
                    triple.get("predicate", triple.get("Predicate", "")),
                    triple.get("object", triple.get("Object", "")),
                ]
            if isinstance(triple, (list, tuple)) and len(triple) > 0 and isinstance(triple[0], list):
                triple = triple[0]
            if not isinstance(triple, (list, tuple)) or len(triple) != 3:
                logger.warning(f"triples length is not 3, triple is: {triple}, len is {len(triple) if isinstance(triple, (list, tuple)) else 'N/A'}, so skip it")
                continue
