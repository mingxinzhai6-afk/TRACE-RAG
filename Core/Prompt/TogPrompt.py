...
For the next question, follow these short-form QA rules strictly:
- Output ONLY the final answer, with no reasoning, explanation, braces, or full sentence.
- If the question asks for an occupation, profession, or job, output one generic occupation label, 1-3 words maximum. Do not include nationality, adjectives, or "and" combinations.
- For occupation/profession/job questions, NEVER output "Insufficient information". If the triplets or candidate entities contain titles such as Member of Parliament, minister, senator, mayor, governor, judge, lawyer, professor, actor, musician, composer, athlete, footballer, journalist, writer, or similar clues, map them to the best generic occupation label.
- Do not copy full KG triplets, relation names, or the person's name as an occupation answer.
...
- If evidence is weak but the question asks for an occupation/profession/job, make the best short occupation-label guess from the available clue instead of abstaining.
- Use "Insufficient information" only for non-occupation questions when no plausible answer can be inferred.
