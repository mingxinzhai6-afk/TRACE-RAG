...
class Evaluator:
    def __init__(self, eval_path: str, dataset_name: str):
        ...
        # dataset_name may be a path like "datasets/Popqa"; extract basename and lowercase
        import os as _os
        dataset_key = _os.path.basename(dataset_name).lower()
        if "narrative" in dataset_key:
            self.mode = "long-narrative"
        else:
            self.mode = self.dataset_mode_map.get(dataset_key, "short-form")
    ...
    def short_eval(self, df: pd.DataFrame):
        ...
        for prediction, answer in zip(pred_list, label_list):
            # Robustly handle non-string predictions/answers (e.g., None / NaN / floats)
            ...
            # F1: use all answers joined (bag-of-words coverage)
            answer_str = " ".join(answer_list)
            # accuracy: 1 if ANY answer option is contained in prediction
            accuracy = 1 if any(
                self.eval_accuracy(prediction_str, ans) for ans in answer_list
            ) else 0
            f1, prec, recall = self.f1_score(prediction_str, answer_str)
            # EM: 1 if prediction exactly matches ANY answer option
            em = 1 if any(
                self.exact_match_score(prediction_str, ans) for ans in answer_list
            ) else 0
            em_list.append(em)
            f1_list.append(f1)
        ...
        # 比例 [0,1]；论文表格多为百分数（×100），两行都打印以免和文献对比时看错量级
        print(f"accuracy: {accuracy:.4f} (ratio) | {accuracy * 100:.4f}%")
        print(f"Precision: {pre:.4f} (ratio) | {pre * 100:.4f}%")
        print(f"Recall: {recall:.4f} (ratio) | {recall * 100:.4f}%")
        print(f"F1: {f1:.4f} (ratio) | {f1 * 100:.4f}%")
        print(f"EM: {em:.4f} (ratio) | {em * 100:.4f}%")
