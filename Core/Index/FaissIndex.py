...
class FaissIndex(BaseIndex):
    ...
    async def _update_index(self, datas: list[dict[str:Any]], meta_data: list):
        ...
        total_texts = len(datas)
        logger.info(f"Preparing {total_texts} items for embedding...")
        documents = []
        texts = []
        # 直接循环构造 Document，避免一次性 asyncio.gather 创建海量任务导致“看起来没进度”
        for i, data in tqdm(enumerate(datas), total=total_texts, desc="Preparing documents", unit="doc", leave=True):
            document = Document(
        ...
        if isinstance(self.embedding_model, OpenAIEmbedding):
            ...
            logger.info(f"Start embedding {total_texts} items in batches of {batch_size}")
            batch_range = range(0, total_texts, batch_size)
            for i in tqdm(batch_range, desc="Embedding", unit="batch", total=(total_texts + batch_size - 1) // batch_size, leave=True):
                batch = texts[i:i + batch_size]
                batch_embeddings = self.embedding_model._get_text_embeddings(batch)
                text_embeddings.extend(batch_embeddings)
            logger.info("Finished embedding all items, building FAISS index...")
        else:
            logger.info(f"Start embedding {total_texts} items")
            text_embeddings = self.embedding_model._get_text_embeddings(texts)
            logger.info("Finished embedding all items, building FAISS index...")

        if not text_embeddings:
            raise ValueError("No embeddings generated for documents, cannot build Faiss index.")

        dim = getattr(self.embedding_model, "dimensions", None)
        if dim is None:
            dim = len(text_embeddings[0])

        vector_store = FaissVectorStore(faiss_index=faiss.IndexHNSWFlat(dim, 32))
        ...
        nodes = []
        for doc, embedding in tqdm(zip(documents, text_embeddings), total=len(documents), desc="Building index nodes", unit="node", leave=True):
