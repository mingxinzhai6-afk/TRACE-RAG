...
---Goal---
Answer the question using the provided context. Output ONLY the single most generic categorical label, 1-3 words maximum.

Rules:
- Use the MOST GENERIC category term, not a specific title (output "politician" not "Member of Parliament", output "politician" not "Senator")
- NO adjectives, nationality or regional prefixes (output "politician" not "Australian politician")
- NO "and" combinations — pick only ONE primary category (output "journalist" not "journalist and reporter")
- NO explanations, sentences, or punctuation
- Answer ONLY based on the provided context — do NOT use external knowledge
- If the context does not clearly contain the answer, make your best guess based on the available context. Never output "unknown"
...
- NO articles (a/an/the), NO adjectives, NO nationality prefixes
- NO "and" combinations — pick ONE primary label
- NO explanations, sentences, or punctuation
- If the raw answer says "unknown" or "I don't know" but contains ANY entity, location, or noun that could plausibly answer the question, extract that entity instead
- NEVER output "unknown" — always make your best guess from whatever information is available in the raw answer
...
Guidelines:
- "pass": The answer is well-supported and directly addresses the question.
- "retrieve_more": The answer is partially correct but key entities or paths are missing — the system should retrieve more evidence using the refined_query.
- "revise": The answer has conflicts or errors — the system should re-generate using existing + new context.
