...
def grouped_bars(ax, data: dict[str, dict[str, list[float | None]]], metric: str) -> None:
    ...
    ax.set_ylabel("Score (%)")
    ax.set_ylim(0, 75 if metric != "F1" else 35)
    ax.set_xticks(x)
...
def main() -> None:
    ...
    fig.suptitle("PopQA Cross-Model Comparison by Method", fontsize=15, fontweight="bold", y=0.992)
    fig.subplots_adjust(top=0.91, bottom=0.07, left=0.06, right=0.99, hspace=0.72)
    fig.savefig(OUT_DIR / "popqa_cross_model_by_method.png", dpi=220)

    fig2, ax2 = plt.subplots(figsize=(15, 5.3))
    grouped_bars(ax2, data, "F1")
    ax2.legend(loc="upper right", frameon=False)
    ax2.set_title("PopQA F1 Cross-Model Comparison by Method", fontsize=14, fontweight="bold")
    fig2.subplots_adjust(top=0.88, bottom=0.28, left=0.06, right=0.99)
    fig2.savefig(OUT_DIR / "popqa_cross_model_f1.png", dpi=220)

    print(OUT_DIR / "popqa_cross_model_by_method.png")
    print(OUT_DIR / "popqa_cross_model_f1.png")
