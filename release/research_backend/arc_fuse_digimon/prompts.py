"""
ARC-Fuse prompts for adaptive graph-text retrieval and refinement.
...
Analyze the question once and output a routing triplet for ARC-Fuse.
...
- If the raw answer indicates missing/unavailable information, output exactly: Insufficient information.
- Preserve complete dates, age ranges, numeric ranges, ordinal rankings, and qualified place/organization names
- Pick the term that most directly and specifically answers the question
- Do not reduce "July 1, 1984" to "1984", "18 - 20" to "18", or "Camden County" to "Camden"
- Output no explanation or sentence
- If no useful information is present, output: Insufficient information.
...
CRITIC_EVALUATE_PROMPT = """You are the Critic in an adaptive graph-text RAG system. Evaluate whether the current answer adequately addresses the question using the retrieved evidence.
...
{{
  ...
  "feedback": {{
    "missing_entities": [<missing entity names>],
    "broken_paths": [<incomplete reasoning chains>],
    "conflicts": [<contradictory evidence pairs>],
    "suggestion": "<what to retrieve or correct next>"
  }},
  "refined_query": "<focused retrieval query>"
}}

Rules:
- pass only when the answer is directly supported and no correction or follow-up retrieval is needed
- retrieve_more when key entities or reasoning paths are missing
- revise when the evidence is relevant but the answer conflicts with or misuses it
- for multi-hop questions, do not pass an answer that only completes the first hop
- if the suggestion asks to retrieve, verify, expand, or find more specific evidence, the verdict cannot be pass
- when verdict is pass, all feedback lists, suggestion, and refined_query must be empty
"""
