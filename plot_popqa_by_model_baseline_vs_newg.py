...
def draw_model_chart(model: str, data: dict[str, dict[str, list[float | None]]]) -> Path:
    ...
    ax.set_title(f"PopQA Baselines vs NewG ({model})", fontsize=14, fontweight="bold")
    ax.set_ylabel("Score (%)")
    ax.set_ylim(0, 76)
    ax.set_xticks(x)
    ...
    out = OUT_DIR / f"popqa_{MODEL_SLUG[model]}_baseline_vs_newg.png"
...
def draw_f1_summary(data: dict[str, dict[str, list[float | None]]]) -> Path:
    ...
    ax.set_title("PopQA F1 Summary: Baselines vs NewG", fontsize=14, fontweight="bold")
    ax.set_ylabel("F1 (%)")
    ax.set_ylim(0, 35)
    ax.set_xticks(x)
    ...
    out = OUT_DIR / "popqa_f1_baseline_vs_newg_summary.png"
    fig.savefig(out, dpi=220)
    plt.close(fig)
    return out
