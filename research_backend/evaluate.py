...
def evaluate_path(result_path, limit: int = 0):
    """Evaluate an ARC-Fuse JSONL result file without loading DIGIMON."""
    df = pd.read_json(result_path, lines=True)
    if limit > 0:
        df = df.head(limit)
    if df.empty:
        raise ValueError(f"No result records found in {result_path}")
    return short_eval(df)
